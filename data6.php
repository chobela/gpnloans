...
$sqlQuery = "SELECT SUM(loans.amount) AS principal, SUM(loans.balance) AS balance, YEAR(loan_date) AS year, MONTH(loan_date) AS month, CONCAT(MONTHNAME(loan_date),', ', YEAR(loan_date)) AS type FROM loans GROUP BY YEAR(loan_date), MONTH(loan_date) ORDER BY year ASC";
